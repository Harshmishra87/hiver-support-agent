...
# --- Join and sample 100 NEW pairs (different seed from before) ---
pairs = brand_replies.merge(customer_tweets, on="in_response_to_tweet_id", how="left")
sample = pairs[["customer_text", "text"]].dropna().sample(100, random_state=7)
sample.columns = ["customer_message", "americanair_reply"]
sample.to_csv("data/processed/americanair_sample_100.csv", index=False)

print(f"Saved 100 pairs (seed=7, different from the earlier 30) to data/processed/americanair_sample_100.csv")